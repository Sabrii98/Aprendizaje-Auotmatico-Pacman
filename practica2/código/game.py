    ...
    def run( self ):
        ...
        while not self.gameOver:
            ...
            if agentIndex == 0:
                ...
                localAction = agent.getAction(self.state)
                localNextState = self.state.generateSuccessor( agentIndex, agent.getAction(self.state))
                localScore = self.state.getScore()
                localNextScore = self.state.generateSuccessor( agentIndex, agent.getAction(self.state)).getScore()
                if (localScore < localNextScore):
                    reward = localNextScore - localScore
                else:
                    reward = 0 
                agent.update(localState, localAction, localNextState, reward)
            move_time = 0
            skip_action = False
